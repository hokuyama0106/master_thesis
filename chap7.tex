\chapter{中央データベースとローカルデータベースの同期}\ref{chap:interfacing_tool}
...
ここで、KEK、LBNLのサーバーにおいてネットワークの混雑状況等の理由による処理時間遅延の影響を調べるために、中央データベースウェブページへの通信時間を1週間通して測定した。
...
ここではファイル添付処理に関しての処理時間検討を行なったが、それ他の処理について処理時間改善方法がないかを考える。
以下の項目について言及する。
\begin{enumerate}
  \item ローカルデータベース内部処理.
